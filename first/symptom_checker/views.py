...
def symptom_checker_api(request):
    if request.method == 'POST':
        ...
        # Filter out empty symptoms and map symptoms to their corresponding indices
        symptoms_mapped = [SYMPTOM_MAPPING.get(symptom, None) for symptom in symptoms if symptom]
        
        # Only proceed if all symptoms are valid
        if None in symptoms_mapped:
            return JsonResponse({'error': 'One or more symptoms are invalid'}, status=400)
...
def predict_disease(symptoms):
    try:
        ...
        input_df = input_df.reindex(sorted(input_df.columns), axis=1).fillna(0)
